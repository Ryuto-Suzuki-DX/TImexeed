...
type AttendanceDayBuilder interface {
	BuildSearchAttendanceDaysQuery(userID uint, req types.SearchAttendanceDaysRequest) (*gorm.DB, results.Result)
	BuildFindAttendanceDayByUserIDAndWorkDateQuery(userID uint, workDate time.Time) (*gorm.DB, results.Result)
