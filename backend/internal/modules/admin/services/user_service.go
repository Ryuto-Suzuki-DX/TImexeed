...
type UserService interface {
	...
	SearchBusinessTargetUsers(req types.SearchBusinessTargetUsersRequest) results.Result
 ...
 * ・削除済みユーザーも検索結果に含めない
 */
func (service *userService) SearchBusinessTargetUsers(req types.SearchBusinessTargetUsersRequest) results.Result {
	normalizedCondition, normalizeResult := utils.NormalizePageSearchCondition(
		utils.PageSearchCondition{
			Keyword: req.Keyword,
			Offset:  req.Offset,
		...
		"SEARCH_BUSINESS_TARGET_USERS_INVALID_OFFSET",
		"検索開始位置が正しくありません",
	)
	if normalizeResult.Error {
		return normalizeResult
	...
	searchQuery, countQuery, buildResult := service.userBuilder.BuildSearchBusinessTargetUsersQuery(req)
	if buildResult.Error {
		return buildResult
	}
	...
	return results.OK(
		types.SearchBusinessTargetUsersResponse{
			Users:   userResponses,
			Total:   total,
			Offset:  req.Offset,
			Limit:   req.Limit,
		...
		"SEARCH_BUSINESS_TARGET_USERS_SUCCESS",
		"業務対象ユーザー一覧を取得しました",
		nil,
	)
}
